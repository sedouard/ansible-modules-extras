...
def split_uri(source_uri):
    # example:
    # https://devopsclestorage3.blob.core.windows.net/vhds/osdiskforlinuxsimple.vhd
    account = source_uri[(source_uri.index('://') + 3):
                         (source_uri.index('blob.core.windows.net')) - 1]
...
def main():
    ...
    if not HAS_DEPS:
        module.fail_json(msg='azure-storage is required for this module')

    (source_account,
     source_container,
     source_blob) = split_uri(module.params.get('source_uri'))

    source = CloudStorageAccount(
        account_name=source_account,
        account_key=module.params.get('source_key'))
    source_service = source.create_block_blob_service()
    dest_service = BlockBlobService(
    ...
    destination_blob = module.params.get('destination_blob')
    ...
    dest_service.create_container(destination_container,
                                  fail_on_exist=False)
    dest_service.copy_blob(destination_container,
                           destination_blob,
                           source_sas_url)
    ...
    if not module.params.get('wait'):
        ...
        operation = dest_service.get_blob_properties(destination_container,
                                                     destination_blob)
        copy = operation.properties.copy
        count = 0
        while copy.status != 'success':
            count = count + 30
            if count > module.params.get('timeout'):
                module.fail_json(msg='Timed out waiting \
                                 for async copy to complete.')
            time.sleep(30)
